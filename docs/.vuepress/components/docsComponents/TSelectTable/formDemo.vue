      ...
      >
        <template #createDeptCode>
          <t-select-table
            ...
            multiple
            placeholder="请选择下拉表格数据"
            :defaultSelectVal="formOpts.formData.createDeptCode"
...
export default {
    ...
    return {
      ...
      formOpts: {
        ...
        fieldList: [
          ...
          { label: '下拉选择表格', slotName: 'createDeptCode' },
  ...
  methods: {
    ...
    edit() {
      this.isShowEditDialog = true
      this.title = '编辑'
      this.formOpts.formData = JSON.parse(localStorage.getItem('formData'))
    ...
    submitForm() {
      this.formOpts.ref.validate((valid) => {
        ...
        localStorage.setItem('formData', JSON.stringify(this.formOpts.formData))
